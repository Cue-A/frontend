...
*출처: Figma `A-01 랜딩페이지 / 기본 - Final` 의 `ProblemSection`(`694:289`) 배경*

시안의 "이런 고민" 구역 배경은 단색이 아니라 **모서리에 깔린 부드러운 블롭 세 개**다.

| 토큰 | HEX | 위치 |
|---|---|---|
| `wash-mint` | `#F4FBFE` | 구역 왼쪽 모서리 |
| `wash-blush` | `#F7F0FD` | 구역 가운데 |
| `wash-lavender` | `#E5E2FC` | 구역 오른쪽 모서리 |

**primary · semantic 스케일과 일부러 분리했다.** 그쪽은 텍스트 · 버튼 · 상태 배지에 쓰는 의미 있는 색인데, 워시는 순수 장식이라 성격이 다르다 (PR #36 리뷰). 셋 다 흰색에 가까워서 **글자 색으로 쓰면 대비가 안 나온다.** 배경에만 쓴다.

**이미지로 내보내지 않는다.** 가로로 꽉 차는 배경이라 뷰포트 폭에 따라 늘어나고, 이미지로 하면 선명하게 만들려고 1x/2x 를 따로 내보내야 한다. `radial-gradient` 세 겹으로 그린다.

화면에서는 `bg-wash` 한 클래스만 붙이면 된다. 바탕(`neutral-50`)과 블롭 위치까지 `src/styles/tokens.css` 안에 들어 있다.

```tsx
<section className="bg-wash px-6 py-20">
```

### 2.6 판 위에 얹히는 면 (Surface)

*출처: Figma `1377:1462` — 장치 테스트 화면의 마이크 레벨 패널 배경*

| 토큰 | HEX | 쓰는 자리 |
|---|---|---|
| `surface-muted` | `#F9F8FF` | 카드 안에 한 겹 더 얹는 보조 패널 |

**워시와 헷갈리기 쉬운데 쓰는 자리가 다르다.** 워시는 화면 **구역 전체**의 장식 배경이고, 서피스는 **카드 안 한 덩어리**의 배경이다. 마이크 레벨 미터처럼 "카드 안에서 여기까지가 한 묶음" 임을 보여줘야 하는 자리에 쓴다.

`neutral` 스케일에 넣지 않았다. 살짝 보라가 도는 흰색이라 나머지 회색과 색감이 어긋난다. `primary` 에도 넣지 않았다. 브랜드 색이 아닌데 그쪽에 들어가면 "누르면 뭔가 되는 색" 으로 읽힌다. 워시를 따로 둔 것과 같은 이유다 (§2.5).

**글자 색으로 쓰지 않는다.** 흰색에 가까워서 대비가 안 나온다. 배경에만 쓴다.

`neutral-50`(`#F6F7FA`)으로 대신하지 않는 이유는 흰 카드 위에서 경계가 거의 안 보이기 때문이다. 그러면 "패널 안에 든 미터" 가 아니라 그냥 떠 있는 막대처럼 보인다 (PR #42 리뷰). 이 토큰을 쓰면 `border` 를 따로 두르지 않아도 한 덩어리로 읽힌다.
