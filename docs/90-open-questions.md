...
### Q13. 🔴 면접관 스타일이 시안 3종, 명세서 2종입니다

| | 값 |
|---|---|
| A-05 시안 · 지금 코드 | 친절한 면접관 · **중립적 면접관** · 압박 면접관 |
| 기능명세서 INT-2 | 친절형 · 압박형 |

명세서 INT-2 는 "면접관 스타일(친절형/압박형)" 이라고 두 종만 적고 있고,
**"난이도는 사용자가 선택하지 않으며 면접관 스타일에 따라 시스템이 배분한다"**
가 이어집니다. 스타일이 난이도 배분의 입력이라면 종류 수가 AI 쪽 동작에
직접 영향을 줍니다.

시안이 낡은 것인지, 명세서가 줄여 적은 것인지 알 수 없어 **임의로 고치지
않았습니다.** 지금 코드는 시안대로 3종입니다.

- 필요한 결정: 2종인지 3종인지. AI 파트에 "중립형" 난이도 배분이 있는지 확인
- 걸리는 작업: `domain/interview/types/sessionSetup.ts` 의 `InterviewerStyle` ·
  `INTERVIEWER_STYLES`, 그리고 A-05 시안
- 연동 전에 정해야 합니다. 세션 생성 요청에 실어 보내는 값입니다

### Q14. 🔴 리포트 점수의 축 구성과 표시 단위가 시안과 명세서에서 다릅니다

기능명세서 REP-2 는 이렇게 적고 있습니다.

> 점수 계산은 내용·말하기·시선 **3개 축**으로 한다. 각 0~100. **마무리는 말하기
> 축의 하위 지표**이며 화면에는 별도 항목으로 표시한다(REP-3). 내부 계산은
> 0~100, **화면 표시는 1~5단계로 변환**하며 두 값 모두 제공한다.

지금 화면(C-01)은 두 가지가 다릅니다.

1. **축이 4개입니다.** 세부 점수에 내용 · 말하기 · 시선 · **답변 마무리** 를 같은
   막대로 그리고, 아래 보조 지표에 답변 마무리를 `4 / 5` 로 **한 번 더** 보여줍니다.
   명세서대로면 세부 점수는 3축이고 마무리는 보조 지표에만 있어야 합니다.
2. **1~5단계 표시가 없습니다.** 화면은 `84 / 100` 처럼 100점 단위만 씁니다.
   시안도 100점 단위로 그려져 있습니다.

둘 다 시안과 명세서가 서로 다른 건이라 임의로 고치지 않았습니다.

- 필요한 결정: 세부 점수를 3축으로 줄일지, 1~5단계를 어디에 어떻게 보여줄지
- 걸리는 작업: `domain/report/components/ScoreSection.tsx`,
  `domain/report/types/report.ts` 의 `ScoreMetric` · `SubMetric`
- 서버 응답 모양에도 영향이 있습니다. 두 값(0~100, 1~5)을 모두 내려주는지
  확인이 필요합니다
