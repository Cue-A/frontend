...
아래 셋은 "실패했습니다" 로 뭉뚱그리면 안 됩니다. 사용자가 다음에 할 행동이 다릅니다.

| `errorCode` | 프론트 처리 |
|---|---|
| `STT_FAILED` | **즉시 재녹음 안내.** 같은 오디오를 다시 보내도 결과가 같습니다 |
| `TTS_FAILED` | **음성 없이 텍스트로 계속 진행.** 질문 텍스트는 이미 만들어져 있습니다 |
| `RESUME_PARSE_FAILED` | 다른 파일 안내 |

`TTS_FAILED` 에서 면접을 중단시키지 마세요. 들리지 않을 뿐 진행할 수 있습니다.
...
### 버튼 · 카드는 `shared/ui` 를 씁니다

시안의 네모난 CTA 와 내용을 담는 판은 `shared/ui/Button` · `shared/ui/Card` 로 그립니다.
화면마다 따로 만들면 같은 버튼인데 크기가 조금씩 달라집니다.

예외는 **모양이 다른 것들**입니다. 문장 속에 섞인 전환 링크("계정이 없으신가요? 회원가입"),
탭처럼 둘 중 하나만 켜지는 띠 — 이런 건 `Button` 의 모양이 아니라서 그리는 쪽에 둡니다.
같은 모양이 두 번째 화면에 생기면 그때 `shared/ui` 로 올립니다.

### 비활성 버튼의 이유는 글로 적습니다

**`disabled` 버튼에 `title` 로 이유를 적지 마세요.** 브라우저는 비활성 요소에 마우스
이벤트를 보내지 않아서 툴팁이 **뜨지 않습니다.** 결과적으로 멀쩡해 보이는데 눌리지 않고,
이유도 아무도 못 보는 버튼이 됩니다.

이유는 버튼 옆이나 아래에 짧은 글로 적습니다. `Button` 이 `disabled:opacity-50` 으로
"지금은 못 누른다"를 보여주고, 글이 "왜 못 누르는지"를 말합니다.

```tsx
// 나쁨 — 툴팁이 안 뜹니다
<Button disabled title="저장 기능은 준비 중이에요">리포트 저장하기</Button>

// 좋음
<Button disabled>리포트 저장하기</Button>
...
**프론트는 AI 결과를 폴링하지 않습니다.** Spring 이 AI 를 1초 간격으로 폴링하고,
프론트에는 **WebSocket 으로 밀어줍니다.** 프론트 폴링은 백엔드 아키텍처에서 이미
기각된 안입니다 — 결과가 프론트로 직행하면 브라우저를 닫는 순간 질문이 저장되지 않습니다.

진행 단계는 백엔드가 `ProgressStage` enum 으로 내려줍니다. **AI 내부 파이프라인 이름
(`stt` / `generating` / `tts`)이 프론트에 오면 안 됩니다.** 그런 값이 보이면 백엔드 버그입니다.
...
**타임아웃은 하나가 아닙니다.** 세션 시작 **90초**, 답변 처리 **60초**.
